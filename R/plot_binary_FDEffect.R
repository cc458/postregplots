...
plot_binary_FDEffect = function(ModelResults, n.sim = 1000, data, clusterid, varname1, varname2,
       ...
       p <- ggplot(simprodicted, aes(x=value, fill=variable)) + theme_tufte() +
              geom_density(alpha=0.4) +  geom_vline(xintercept = 0, colour = gray(1/2), lty = 2) +
              theme(legend.position="bottom") + labs(x = "First Difference", caption =  paste0("Note: density plots are based on ", n.sim," MC iterations")) +
              scale_fill_manual(legend_title, values = c("gold","blue", "red"))
       return(p)
